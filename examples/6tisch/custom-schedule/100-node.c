...
static void
initialize_tsch_schedule_2(uint16_t id_node)
{
  int i, j;
  ...
  for (i = 0; i < 2 - 1; ++i)
  {
    ...
    // {
    channel_offset = 0;
    tsch_schedule_add_link(sf_2, LINK_OPTION_TX, LINK_TYPE_NORMAL, &addr, id_node * 2, channel_offset, 1);
...
initialize_coord_schedule(void)
{
  ...
  for (i = 0; i < 32; ++i)
  {
    ...
    // sf_2[i] = tsch_schedule_add_slotframe(remote_id, 256);
    for (j = 0; j < sizeof(addr); j += 2)
    {
      addr.u8[j + 1] = remote_id & 0xff;
      addr.u8[j + 0] = remote_id >> 8;
    ...
    channel_offset = 0;
    slot_offset = remote_id * 2;
    tsch_schedule_add_link(sf_2, LINK_OPTION_RX, LINK_TYPE_NORMAL, &addr, slot_offset, channel_offset, 1);
    // tsch_schedule_add_link(sf_2, LINK_OPTION_RX, LINK_TYPE_NORMAL, &addr, 15, channel_offset, 0);
  }
          ...
          uint16_t datalen)
{
  ...
  if (datalen >= sizeof(id_node))
  {
    memcpy(&id_node, data, sizeof(id_node));

    LOG_INFO("Received from ");
    // LOG_INFO(node_id);
    LOG_INFO_6ADDR(sender_addr);
    LOG_INFO_(", node_id %" PRIu16 "\n", id_node);
  }
}
...
PROCESS_THREAD(node_process, ev, data)
{
  ...
  // static struct etimer periodic_timer; periodic_timer_32, periodic_timer_64,
  static struct etimer periodic_timer_128;
  ...
  { /* Running on the root? */
    ...
    initialize_tsch_schedule_2(node_id);
  }
  NETSTACK_MAC.on();
  ...
  while (1)
  {
    ...
    if (etimer_expired(&periodic_timer_128))
    {
      if (NETSTACK_ROUTING.node_is_reachable() && NETSTACK_ROUTING.get_root_ipaddr(&dst))
      {
        ...
        if (node_id != 1)
        {
          ...
          LOG_INFO("Send to ");
          LOG_INFO_6ADDR(&dst);
          LOG_INFO_(", seqnum %" PRIu16 "\n", node_id);
          // simple_udp_sendto(&udp_conn, &seqnum, sizeof(seqnum), &dst);
          simple_udp_sendto(&udp_conn, &node_id, sizeof(node_id), &dst);
        }
      }
      etimer_set(&periodic_timer_128, SEND_INTERVAL_128);
    }
