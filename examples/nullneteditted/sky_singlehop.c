...
#define PRINT_INTERVAL (30 * CLOCK_SECOND)
...
// void
// wirelesshart_callback_new_time_source(void)
// {
...
//     {10, {{0x78, 0x2d, 0x24, 0x10, 0x00, 0x74, 0x12, 0x00}}}, // 21
//     {11, {{0x53, 0xff, 0x60, 0x11, 0x00, 0x74, 0x12, 0x00}}}, // 87
//     {12, {{0xbf, 0x63, 0x5a, 0x0c, 0x00, 0x74, 0x12, 0x00}}}, //
...
install_node_schedule(uint16_t id_node)
{
  ...
  // add coordinator to the table;
  tsch_queue_add_nbr(&coordinator_addr);
  tsch_queue_update_time_source(&coordinator_addr);

  struct tsch_slotframe *sf_tx = tsch_schedule_add_slotframe(2, TX_SLOTFRAME_SIZE);
  // struct tsch_slotframe *sf_tx_2 = tsch_schedule_add_slotframe(3, 512);
  // first transmission
  ...
  //   if (curr->id == hop_id)
  //   {
  //     struct tsch_slotframe *sf_hop = tsch_schedule_add_slotframe(3, TX_SLOTFRAME_SIZE);
...
initialize_coord_schedule(void)
{
  ...
  while (curr->id !=0)
  {
    ...
    // tsch_schedule_add_link(sf_tx_2, LINK_OPTION_RX, LINK_TYPE_NORMAL, &node_addr, slot_offset + 256, channel_offset, 0);
...
{
  static struct etimer periodic_timer, periodic_timer_minute; //periodic_timer_512, 
  ...
  LOG_INFO("Nullnet_buff %d ", sizeof(nullnet_buf));
  ...
  if (!linkaddr_cmp(&dest_addr, &linkaddr_node_addr))
  {
    ...
    // etimer_set(&periodic_timer_512, SEND_INTERVAL_512);
    etimer_set(&periodic_timer_minute, PRINT_INTERVAL);
    ...
    while (1)
    {
      ...
      else if (etimer_expired(&periodic_timer_minute))
      {

        LOG_INFO("Throughput per minute %u to ", throughput);

        LOG_INFO_("\n");
        throughput = 0;
        etimer_reset(&periodic_timer_minute);
      }
    }
  }
  else
  {
    ...
    while (1)
    {
      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&periodic_timer_minute));

      LOG_INFO("Throughput per minute %u ", throughput);
