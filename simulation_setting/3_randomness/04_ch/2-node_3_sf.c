...
initialize_tsch_schedule(void)
{
  ...
  for (i = 0; i < 2 - 1; ++i)
  {
    ...
    tsch_schedule_add_link(sf_32, LINK_OPTION_TX, LINK_TYPE_NORMAL, &addr, 4, 1, 0);
    tsch_schedule_add_link(sf_128, LINK_OPTION_TX, LINK_TYPE_NORMAL, &addr, 11, 2, 0);
...
initialize_coord_schedule(void)
{
  ...
  for (i = 0; i < 2 - 1; ++i)
  {
    ...
    tsch_schedule_add_link(sf_32, LINK_OPTION_RX, LINK_TYPE_NORMAL, &addr, 4, 1, 0);
    tsch_schedule_add_link(sf_128, LINK_OPTION_RX, LINK_TYPE_NORMAL, &addr, 11, 2, 0);
