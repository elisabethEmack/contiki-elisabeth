...
#define SEND_INTERVAL (1024 * CLOCK_SECOND / 100)
...
static void
initialize_tsch_schedule(uint16_t node_idx)
{
  int i, j;
  ...
  for (i = 0; i < 2 - 1; ++i)
  {
    ...
    slot_offset = node_idx + 2;
    tsch_schedule_add_link(sf_common, LINK_OPTION_TX, LINK_TYPE_NORMAL, &addr, slot_offset, channel_offset, 0);
...
initialize_coord_schedule(void)
{
  ...
  for (i = 0; i < 100; ++i)
  {
    ...
    // uint16_t remote_id = i + 1;
    uint16_t remote_id = i + 1;
    sf[i] = tsch_schedule_add_slotframe( i+1 , APP_SLOTFRAME_SIZE);
    ...
    tsch_schedule_add_link(sf[i], LINK_OPTION_RX, LINK_TYPE_NORMAL, &addr, slot_offset, channel_offset, 0);
